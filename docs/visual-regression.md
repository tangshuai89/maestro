...
`maxDiffPixelRatio` 现在是 **0.005（0.5%）**。这个值是**两个实测数字夹出来的窗口**：

| 来源 | 数值 |
|---|---|
| **跨机器噪声**（本地 macOS ↔ GitHub macOS runner） | 0.29% / 0.29% / 0.29% / 0.39%（四档，差异**全在文字抗锯齿**上；背景/渐变/星尘逐像素一致） |
| **真漂移**（故障注入：`.th-hud` 从 `top:24` 挪到 `32`，8px） | 0.69% |

三种容差各自的下场：

- **0.1%** → 跨机器文字 AA 就天天假红（PR #86 实测：4 例全红、2 例 SourceSelect 绿）；
- **1%** → 8px 漂移只差约 9,000 px（0.69%），**会被放过**（本地故障注入实测）；
- **0.5%** → 两边的窗口都留住了 ✅

**结论一：加完门禁一定要做一次"故意改坏"的自检**，否则你不知道它是真在守还是在装样子。
**结论二：容差不是越紧越好** —— 紧到跨机器噪声以下，门禁就从"守门"变成"天天误报"，
最后必然被人 `--update` 掉。真需要更灵敏，正确做法是**固定渲染环境（Docker）**，不是继续收紧。

顺带：自检还能告诉你"哪些尺寸有覆盖规则把改动吃掉了"（narrow 档把 `.th-hud` 钉在 16px，
