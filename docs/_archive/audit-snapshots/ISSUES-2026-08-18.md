...
> ⚠️ **历史快照（2026-08-18 → 已 ~1 月）**。文档中至少以下条目与现状不符：
>
> | 条目 | 文档说 | 现状（2026-09-20 抽查） |
> |---|---|---|
> | §1.1 [CRIT] | `like.e2e.test.ts` 用 `app.listen(0)`，sandbox 下 EPERM | 已迁 `InProcessClient` + `getRequestHandlerFromNestApp`（`packages/server/src/test-helpers/in-process-http.ts`），README.md:271 也明示"sandbox 友好" |
> | §2.7 [HIGH] | `parseLrc` 多 tag / 时间越界 bug | 已在 `d014cf4` 修复 + `lyrics.test.ts` 覆盖 |
> | §3.5 [LOW] | `package.json:5` "A minimal music player for Mac..." | 现已改为「Cross-platform music player (QQ / NetEase / Deezer / Spotify)…」（line 5） |
> | §6.1 [LOW] | README 没提 npm test | README.md:261、README.zh-CN.md:246、README.ja.md 同位置均已写明 |
> | §3.6 [LOW] | AGENTS.md 路径与实际不符 | AGENTS.md 已重写为 CLAUDE.md 指针（2026-09-20） |
>
> **维护建议**：要么按上方表格把"已修"标 ✅ + 留剩余条，要么归档到 `docs/_archive/` 重新生成。
> **不要**继续当现役 TODO——半打条目已自相矛盾。
