...
- [x] 搜索得到一首在 QQ + 网易云 + Deezer 都有版权的歌 → 点 ❤ → 三个平台本地 liked 都加进去
- [x] 网易云已登录：网易云那边真的能在「我喜欢的音乐」歌单里看到
- [x] QQ 未登录或没公开 ❤ API：只本地记录，不报错
  > ⚠️ 已修订（见 [`heart-sync`](../heart-sync/spec.md)）：Deezer 是匿名源、无 per-user
  > library，**不再本地记录红心**（结构性排除），避免角标虚高 + 假红心污染。
- [x] 再点 ❤ 一次：三个平台本地 liked 都清掉；网易云「我喜欢的音乐」里同步移除
- [x] 单平台未登录（搜索结果里只有 QQ 一份）：点 ❤ 行为等同现在的 toggleLike，不报错
- [x] 当前正在播的歌曲 ❤ 状态显示：已 fan-out 时，❤ 图标高亮 + 后面带"3❤"小角标
- [x] 平台搜索失败的 source 不写入 liked 集合（避免幽灵态）
- [x] `fanOut` 状态持久化到 .storage/state.json，重启后保留
